...
- With Screen Recording granted, cover each display and verify there is one frozen snapshot per display: video, clocks, and moving windows behind it remain static until uncovering. Confirm it is scaled to a reasonable low-resolution blur, has a Gaussian-style blur matching the Blur level slider (about 12 screen points at the middle, up to 18 at the far left, 6 at the far right), has no white tint, and centers the app icon/name on each display in light and dark appearances.
- Drag the Blur level slider while a cover is visible and confirm the cover stays visible during the drag and then re-blurs without exposing the desktop; confirm the choice survives relaunch. The raw image must not be visible in the cover, retained after uncovering, written to files, or sent over the network.
...
- Verify VoiceOver describes the menu item, status, and both sliders (Tolerance and Blur level); operate the menu with the keyboard.
